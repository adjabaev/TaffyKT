...
class TrackSizing {
    ...
    companion object {
        ...
        fun determineIfItemCrossesFlexibleOrIntrinsicTracks(
            ...
            for (item in items) {
                ...
                item.crossesIntrinsicColumn =
                    item.trackRangeExcludingLines(AbstractAxis.INLINE)
                        .any { i -> columns[i].hasIntrinsicSizingFunction() }
        ...
        fun trackSizingAlgorithm(
            ...
            if (otherAxisTracks.size > 3) {
                val len = otherAxisTracks.size
                val innerGutterTracks =
                    otherAxisTracks.slice(2 until len)
                        .filterIndexed { index, track -> index % 2 == 0 && !track.isCollapsed }
            ...
            stretchAutoTracks(axis, axisTracks, axisMinSize, availableGridSpace)
        }
        ...
        fun initializeTrackSizes(axisTracks: List<GridTrack>, axisInnerNodeSize: Option<Float>) {
            val lastTrackIdx = axisTracks.size - 1

            // First and last grid lines are always zero-sized.
            axisTracks[0].baseSize = 0.0f
            axisTracks[0].growthLimit = 0.0f
            axisTracks[lastTrackIdx].baseSize = 0.0f
            axisTracks[lastTrackIdx].growthLimit = 0.0f

            val allButFirstAndLast = 1 until lastTrackIdx
            for (track in axisTracks.slice(allButFirstAndLast)) {
                ...
                track.baseSize = track.minTrackSizingFunction.definiteValue(axisInnerNodeSize).unwrapOr(0.0f)
        ...
        fun resolveItemBaselines(
            ...
            items.sortedBy { item -> item.placement(otherAxis).start }
            ...
            while (remainingItems.isNotEmpty()) {
                ...
                val nextRowFirstItem = remainingItems.position { item -> item.placement(otherAxis).start != currentRow }
                ...
                val rowItems = if (nextRowFirstItem.isSome()) {
                    val index = nextRowFirstItem.unwrap()
                    val (rowItems, tail) = remainingItems.splitAt(index)
                    remainingItems = tail
                    rowItems
                } else {
                    val rowItems = remainingItems
                    remainingItems = ArrayList()
                    rowItems
                }
                ...
                val rowBaselineItemCount = rowItems.count { item -> item.alignSelf == AlignSelf.BASELINE }
                ...
                for (item in rowItems) {
                    ...
                    item.baseline =
                        Option.Some(baseline.unwrapOr(height) + item.margin.top.resolveOrZero(innerNodeSize.width))
                }

                // Compute the max baseline of all items in the row
                val rowMaxBaseline =
                    rowItems.map { item -> item.baseline.unwrapOr(0.0f) }.maxByRs { a, b -> a.compareTo(b) }.unwrap()

                // Compute the baseline shim for each item in the row
                for (item in rowItems) {
                    item.baselineShim = rowMaxBaseline - item.baseline.unwrapOr(0.0f)
                }
            }
        }
        ...
        fun resolveIntrinsicTrackSizes(
            ...
            // Already done at this point. See resolveItemBaselines function.
            ...
            val itemSizer = IntrisicSizeMeasurer(
                tree = tree,
            ...
            while (true) {
                val b = batchedItemIterator.next(items)
                if (b.isNone()) {
                    break
                }

                val (batch, isFlex) = b.unwrap()
                ...
                if (!isFlex && batchSpan == 1) {
                    for (item in batch) {
                        ...
                        // Handle base sizes
                        val t = track.minTrackSizingFunction
                        val newBaseSize = when {
                            t is MinTrackSizingFunction.MinContent -> {
                            ...
                            t is MinTrackSizingFunction.Fixed && t.l is LengthPercentage.Percent -> {
                            ...
                            t is MinTrackSizingFunction.MaxContent -> {
                                f32Max(track.baseSize, itemSizer.maxContentContribution(item))
                            }

                            t is MinTrackSizingFunction.Auto -> {
                                val space = when (axisAvailableGridSpace) {
                                    ...
                                    else -> itemSizer.minimumContribution(item, axisTracks)
                                }
                        ...
                        if (track.maxTrackSizingFunction is MaxTrackSizingFunction.FitContent) {
                            ...
                            val maxContentContribution = f32Min(itemSizer.maxContentContribution(item), fitContentLimit)
                ...
                val useFlexFactorForDistribution = isFlex && flexFactorSum != 0.0f

                // 1. For intrinsic minimums:
                // First increase the base size of tracks with an intrinsic min track sizing function
                val hasIntrinsicMinTrackSizingFunction: (GridTrack) -> Boolean = { track ->
                ...
                val hasMinOrMaxContentMinTrackSizingFunction = { track: GridTrack ->
                    track.minTrackSizingFunction is MinTrackSizingFunction.MinContent || track.minTrackSizingFunction is MinTrackSizingFunction.MaxContent
                }
                for (item in batch) {
                ...
                // Define fitContentLimitedGrowthLimit function. This is passed to the distributeSpaceUpToLimits
                ...
                if (axisAvailableGridSpace == AvailableSpace.MaxContent) {
                    /**
                     * Whether a track:
                     ...
                     */
                    fun hasAutoMinTrackSizingFunction(track: GridTrack): Boolean {
                        return track.minTrackSizingFunction == MinTrackSizingFunction.Auto &&
                                track.maxTrackSizingFunction != MaxTrackSizingFunction.MinContent
                    }

                    /**
                     * Whether a track has a MaxContent min track sizing function
                     */
                    fun hasMaxContentMinTrackSizingFunction(track: GridTrack): Boolean {
                        return track.minTrackSizingFunction == MinTrackSizingFunction.MaxContent
                    }
                    ...
                    for (item in batch) {
                        ...
                        if (space > 0.0f) {
                            ...
                            if (tracks.any { track -> hasMaxContentMinTrackSizingFunction(track) }) {
                                distributeItemSpaceToBaseSize(
                                    isFlex,
                                    useFlexFactorForDistribution,
                                    space,
                                    tracks,
                                    { track -> hasMaxContentMinTrackSizingFunction(track) },
                                    { _ -> Float.POSITIVE_INFINITY },
                                    IntrinsicContributionType.MAXIMUM
                                )
                            } else {
                                val fitContentLimitedGrowthLimit =
                                    { track: GridTrack -> track.fitContentLimitedGrowthLimit(axisInnerNodeSize) }
                                distributeItemSpaceToBaseSize(
                                    isFlex,
                                    useFlexFactorForDistribution,
                                    space,
                                    tracks,
                                    { track -> hasAutoMinTrackSizingFunction(track) },
                ...
                val hasMaxContentMinTrackSizingFunction =
                    { track: GridTrack -> track.minTrackSizingFunction is MinTrackSizingFunction.MaxContent }
                for (item in batch) {
                    val axisMaxContentSize = itemSizer.maxContentContribution(item)
        ...
        fun distributeItemSpaceToBaseSize(
            ...
            fun distributeItemSpaceToBaseSizeInner(
                ...
                if ((space == 0.0f) || !tracks.any { track -> trackIsAffected(track) }) {
                    return
                }
                ...
                val trackSizes = tracks.map { track -> track.baseSize }.sum()
                var extraSpace = f32Max(0.0f, space - trackSizes)
                ...
                if (extraSpace > threshold) {
                    ...
                    var filter = when (intrinsicContributionType) {
                        IntrinsicContributionType.MINIMUM -> {
                            { track: GridTrack -> track.maxTrackSizingFunction.isIntrinsic() }
                        }
                    ...
                    val numberOfTracks =
                        tracks.filter { track -> trackIsAffected(track) }.count { track -> filter(track) }
                    if (numberOfTracks == 0) {
                        filter = { _: GridTrack -> true }
                    }
        ...
        fun distributeItemSpaceToGrowthLimit(
            ...
            //   - no space to distribute
            //   - no affected tracks to distribute space to
            if (space == 0.0f || tracks.none { track -> trackIsAffected(track) }) {
                return
            }

            // 1. Find the space to distribute
            val trackSizes = tracks
                .map { track ->
                    if (track.growthLimit == Float.POSITIVE_INFINITY) {
                        track.baseSize
                    } else {
                        track.growthLimit
                    }
                ...
                .sum()
            val extraSpace = f32Max(0.0f, space - trackSizes)
            ...
            val numberOfGrowableTracks = tracks
                .filter { track -> trackIsAffected(track) }
                .count { track ->
            ...
            if (numberOfGrowableTracks > 0) {
                ...
                for (track in tracks.filter { track -> trackIsAffected(track) }.filter { track ->
                    track.infinitelyGrowable || track.fitContentLimitedGrowthLimit(axisInnerNodeSize) == Float.POSITIVE_INFINITY
                }) {
                ...
                // If space remains after all tracks are frozen, unfreeze and continue to distribute space to the
                // item-incurred increase when handling any intrinsic growth limit: all affected tracks.
                ...
                distributeSpaceUpToLimits(
                    ...
                    { track: GridTrack -> track.fitContentLimit(axisInnerNodeSize) }
                )
            }
            // 4. For each affected track, if the track’s item-incurred increase is larger than the track’s planned increase
            // set the track’s planned increase to that value.
        ...
        fun maximiseTracks(
            ...
            val usedSpace = axisTracks.map { track -> track.baseSize }.sum()
        ...
        fun expandFlexibleTracks(
            ...
            val flexFraction = when (val availableSpace = availableGridSpace.get(axis)) {
                ...
                is AvailableSpace.Definite -> {
                    val availableSpace = availableSpace.availableSpace
                    val usedSpace = axisTracks.map { track -> track.baseSize }.sum()
                ...
                is AvailableSpace.MinContent -> 0.0f
                ...
                is AvailableSpace.MaxContent -> {
                    ...
                    val flexFraction = f32Max(
                        ...
                        axisTracks.filter { track -> track.maxTrackSizingFunction.isFlexible() }
                            .map { track ->
                            ...
                            .maxByRs { a, b -> Ordering.totalCmp(a, b) }
                            .unwrapOr(0.0f),
                        ...
                        items.filter { item -> item.crossesFlexibleTrack(axis) }
                            .map { item ->
                                val tracks = axisTracks.slice(item.trackRangeExcludingLines(axis))
                    ...
                    val hypotheticalGridSize = axisTracks
                        .map { track ->
                            when (val mx = track.maxTrackSizingFunction) {
                                is MaxTrackSizingFunction.Flex -> {
                                    val trackFlexFactor = mx.f
                                    f32Max(track.baseSize, trackFlexFactor * flexFraction)
                                }
                        ...
                        .sum()
                    val axisMinSize = axisMinSize.unwrapOr(0.0f)
            ...
            for (track in axisTracks) {
                if (track.maxTrackSizingFunction.isFlexible()) {
                    track.baseSize = f32Max(
                        track.baseSize,
                        (track.maxTrackSizingFunction as MaxTrackSizingFunction.Flex).f * flexFraction
                    )
                }
            }
        }
        ...
        fun findSizeOfFr(tracks: List<GridTrack>, spaceToFill: Float): Float {
            ...
            while (true) {
                ...
                var usedSpace = 0.0f
                var naiveFlexFactorSum = 0.0f
                for (track in tracks) {
                    val lt = track.maxTrackSizingFunction
                    when {
                        // Tracks for which flex_factor * hypothetical_fr_size < track.base_size are treated as inflexible
                        (lt is MaxTrackSizingFunction.Flex && lt.f * hypotheticalFrSize >= track.baseSize) -> naiveFlexFactorSum += lt.f
                        else -> usedSpace += track.baseSize
                    }
                }
                val leftoverSpace = spaceToFill - usedSpace
                val flexFactor = f32Max(naiveFlexFactorSum, 1.0f)
                ...
                val hypothericalFrSizeIsValid = tracks.all { track ->
                    when (track.maxTrackSizingFunction) {
                        is MaxTrackSizingFunction.Flex -> {
                            val flexFactor = (track.maxTrackSizingFunction as MaxTrackSizingFunction.Flex).f
                            flexFactor * hypotheticalFrSize >= track.baseSize || flexFactor * previousIterHypotheticalFrSize < track.baseSize
                        }

                        else -> true
                    }
                }
                if (hypothericalFrSizeIsValid) {
                    break
                }
            }
         ...
         * definite free space equally amongst them.
         */
        fun stretchAutoTracks(
            ...
            val numAutoTracks =
                axisTracks.count { track -> track.maxTrackSizingFunction == MaxTrackSizingFunction.Auto }
            if (numAutoTracks > 0) {
                val usedSpace = axisTracks.map { track -> track.baseSize }.sum()
                ...
                val freeSpace = if (availableGridSpace.get(axis).isDefinite()) {
                    availableGridSpace.get(axis).computeFreeSpace(usedSpace)
                } else {
                    when {
                        axisMinSize.isSome() -> axisMinSize.unwrap() - usedSpace
                        else -> 0.0f
                    }
                }
                if (freeSpace > 0.0) {
                    val extraSpacePerAutoTrack = freeSpace / numAutoTracks.toFloat()
                    axisTracks.filter { track -> track.maxTrackSizingFunction == MaxTrackSizingFunction.Auto }
                        .forEach { track ->
        ...
        fun distributeSpaceUpToLimits(
            ...
            while (spaceToDistribute > threshold) {
                val trackDistributionProportionSum: Float = tracks
                    ...
                    .filter { track -> trackIsAffected(track) }
                    .map { track -> trackDistributionProportion(track) }
                    .sum()

                if (trackDistributionProportionSum == 0f) {
                    break
                }

                // Compute item-incurred increase for this iteration
                val minIncreaseLimit =
                    tracks
                        .filter { track ->
                            trackAffectedProperty(track) + track.itemIncurredIncrease < trackLimit(
                                track
                            )
                        }
                        .filter { track -> trackIsAffected(track) }
                        .map { track ->
                            (trackLimit(track) - trackAffectedProperty(track)) / trackDistributionProportion(track)
                        }
                        .minByRs { a, b ->
                            Ordering.totalCmp(
                                a,
                ...
                for (track in tracks
                    ...
                    if (increase > 0.0f && trackAffectedProperty(track) + increase <= trackLimit(track)) {
